...
int Seat_Srv_FetchValidByRoomID(seat_list_t list, int roomID) {
    int count = Seat_Perst_SelectByRoomID(list, roomID);
    // 安全遍历删除无效节点
    seat_list_node_t* p = (seat_list_node_t*)list->node.next;
    seat_list_node_t* next;
    while (p != list) {
        next = (seat_list_node_t*)p->node.next;  // 保存下一个节点
...
void Seat_Srv_SortSeatList(seat_list_t list) {
    ...
    list->node.prev->next = NULL;   // 尾节点的next原指向头，现在置NULL
    seat_list_node_t* listLeft = (seat_list_node_t*)list->node.next;  // 第一个数据节点
    seat_list_node_t* p;
    // 清空原链表（重新初始化头结点）
    ...
    while (listLeft != NULL) {
        p = listLeft;
        listLeft = (seat_list_node_t*)listLeft->node.next;  // 保存下一个节点
