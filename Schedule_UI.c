    ...
    while (1) {
        ...
        case 'T':
            ...
            if (Schedule_Srv_FetchByID(schedule_id, &schedule_data)) {
                ...
                if (schedule_list != NULL) {
                    // 释放整个链表
                    schedule_list_node_t* current = (schedule_list_node_t*)schedule_list->node.next;
                    while (current != (list_node_t*)schedule_list) {
                        schedule_list_node_t* next = (schedule_list_node_t*)current->node.next;
                        free(current);
                        current = next;
                    }
                    // 释放头节点
        ...
        case 'P':
            ...
            else if (Pageing_IsFirstPage(paging)) {
                printf("已经是第一页了！\n");
            }
            break;
        ...
        case 'N':
            ...
            else if (Pageing_IsLastPage(paging)) {
                printf("已经是最后一页了！\n");
            }
            break;
...
int Schedule_UI_Add(int play_id) {
    ...
    if (sscanf(input, "%d", &new_schedule.studio_id) != 1) {
        printf("输入格式错误！请输入一个有效的数字。\n");
        printf("按任意键继续...");
        getchar();
        return 0;
    }
    ...
    if (sscanf(input, "%d-%d-%d", &year, &month, &day) != 3) {
        printf("日期格式错误！请使用yyyy-mm-dd格式，例如: 2024-12-25\n");
        printf("按任意键继续...");
        getchar();
        return 0;
    }
    ...
    if (sscanf(input, "%d:%d", &hour, &minute) != 2) {
        printf("时间格式错误！请使用hh:mm格式，例如: 14:30\n");
        printf("按任意键继续...");
        getchar();
        return 0;
    }
    ...
    if (sscanf(input, "%d", &new_schedule.seat_count) != 1) {
        printf("输入格式错误！请输入一个有效的数字。\n");
        printf("按任意键继续...");
        getchar();
        return 0;
    ...
    if (Schedule_Srv_Add(&new_schedule)) {
        ...
        printf("演出厅ID: %d\n", new_schedule.studio_id);
        printf("演出厅名称: %s\n", studio_info.name);
        printf("演出厅座位行数: %d\n", studio_info.rowsCount);
        printf("演出厅座位列数: %d\n", studio_info.colsCount);
        printf("演出厅容量: %d\n", studio_info.seatsCount);
        printf("演出日期: %04d-%02d-%02d\n",
            new_schedule.date.year, new_schedule.date.month, new_schedule.date.day);
        printf("演出时间: %02d:%02d\n",
